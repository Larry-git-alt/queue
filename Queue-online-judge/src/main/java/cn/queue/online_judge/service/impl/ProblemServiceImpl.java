...
//    }
    ...
    public PageBean page(Integer page, Integer pageSize, String tags, String title, Short difficulty) {
        ...
        //2、执行查询
        List<Problem> proList = problemMapper.list(tags,title,difficulty);
        Long userId = 1L;

        List<Relation> relations = relationMapper.selectList(new LambdaQueryWrapper<Relation>().eq(Relation::getUserId, userId).isNotNull(Relation::getProblemId));
        for (Problem p : proList) {
            for (Relation r : relations) {
                if (Objects.equals(p.getId(), r.getProblemId()))
                    p.setStatus("通过");
            }
        }
        Page<Problem> p = (Page<Problem>) proList;
